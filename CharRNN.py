...
class CharRNN():
    ...
    def rnn_layer(self):
        ...
        #########################################################################################################
        def cell(type, rnn_size, keep_prob):
            if type == 'LSTM':
                rnn_cell = tf.nn.rnn_cell.LSTMCell(rnn_size)
            else:
        ...
        rnn = tf.nn.rnn_cell.MultiRNNCell(
            [cell(self.cell_type, self.rnn_size, self.keep_prob) for _ in range(self.num_layers)])
        self.initial_state = rnn.zero_state(self.num_steps, tf.float32)
    ...
    def my_optimizer(self):
        ...
        self.optimizer = tf.train.AdamOptimizer(self.learning_rate).apply_gradients(zip(grads, variables))
        ...
        with self.session as sess:
            ...
            for x, y in batches:
                ...
                if (counter % save_every_n == 0):
                    self.saver.save(sess, "checkpoints_GRU/i{}_l{}.ckpt".format(counter, self.rnn_size))
    ...
    def sample(self, checkpoint, n_samples, vocab_size, vocab_to_ind, ind_to_vocab, prime='You \n'):
        ...
        with self.session as sess:
            ...
            for char in prime:
                ind = vocab_to_ind[char]
                x = np.array([[ind]])
                feed = {self.inputs: x,
                        self.keep_prob: 1.0,
                        self.initial_state: new_state}
                preds, new_state = sess.run([self.prob_pred, self.final_state],
                                            feed_dict=feed)

            ind = pick_top_n(preds, vocab_size)
            samples.append(ind_to_vocab[ind])
